            ...
            {labels.map((label) => {
                return (
                    <label key={label} className='custom-control custom-checkbox mb-2 mr-2'>
                        <input type='checkbox' id={label} value={label} defaultChecked={defaultLabels.indexOf(label) > -1} className='custom-control-input' onChange={(event) => onChange(event.target.value)} />
